...
**Bundle** — 针对某个技术栈或领域预组合的 agent、skill、rule、MCP server 集合。Bundle 可叠加：一个全栈项目可以组合 `ts-backend-proj` + `ts-frontend-proj` + `database`。所有语言 bundle 自动包含 `core`（planner、architect、tdd 等通用能力）。
