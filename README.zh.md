...
- **Hook + MCP 集成** — ECC 的 26 条 hook 按 profile 精细控制 + MCP server 自动合并
- **自动依赖检测** — 扫描 `/command` 和 `mcp__server__` 引用，自动带入
...
不写配置文件时使用默认值：`core` bundle（软件开发通用能力）、standard hook profile、claude-mem 首次 apply 自动检测。完整配置项见[配置参考](#配置参考)。

## Slash Command

`apply` 会自动安装 `/ecc-tailor` 斜杠命令到 `~/.claude/commands/`。在 Claude Code 会话里直接用自然语言，不用记命令：

```
/ecc-tailor 给当前项目加上 hexagonal-architecture
/ecc-tailor 扫描一下这个项目应该装什么
/ecc-tailor 看看状态
/ecc-tailor 有哪些 skill 我还没用？
```

斜杠命令覆盖所有 CLI 功能。Claude 会把你的意图翻译成对应命令，执行后总结结果。

## 核心概念

**Bundle** — 针对某个技术栈或领域预组合的 agent、skill、rule、MCP server 集合。Bundle 可叠加：一个全栈项目可以组合 `ts-backend-proj` + `ts-frontend-proj` + `database`。所有语言 bundle 自动包含 `core`（planner、code-reviewer、tdd 等通用能力）。

**两级安装** — 通用 bundle（如 `core`）装到 `~/.claude/`（所有项目可用）。栈相关 bundle 装到 `<project>/.claude/`（仅在该项目生效）。

**Symlink 模式** — agent、skill、rule 通过 symlink 指向 ECC clone，不做文件复制。ECC `git pull` 后所有内容即时更新。用 `ecc-tailor fork <path>` 可以把某个文件脱离 symlink，自由定制。
...
<details>
<summary><b>语言 / 框架</b></summary>

| Bundle | Agents | Skills | 适用场景 |
|---|---|---|---|
| `core` | 4 | 2 | 软件开发通用能力（planner、code-reviewer、tdd 等） |
...
<details>
<summary><b>领域 / 功能</b></summary>

| Bundle | Agents | Skills | 适用场景 |
|---|---|---|---|
| `ai-app-dev` | 0 | 5 | Claude API / MCP server / Claude Code 扩展 |
| `security` | 2 | 3 | 安全审查 + 漏洞扫描 |
| `database` | 1 | 3 | 数据库设计、迁移、优化 |
| `devops` | 1 | 5 | Docker、部署、基准测试 |
| `healthcare` | 1 | 5 | 医疗 / HIPAA / 临床系统 |
| `opensource` | 3 | 1 | Fork、脱敏、打包开源发布 |
| `a11y` | 1 | 1 | 无障碍 / WCAG 合规 |
| `seo` | 1 | 1 | SEO 审计与优化 |
| `gan-harness` | 3 | 1 | GAN 多 agent 对抗生成 |
| `agent-dev` | 2 | 16 | AI agent 开发与编排 |
| `research` | 2 | 6 | 深度调研、网络搜索、市场分析 |
| `content` | 1 | 13 | 写作、视频、社交媒体、演示文稿 |
| `ops` | 2 | 12 | 邮件、Jira、GitHub、Slack 工作流自动化 |
| `crypto` | 0 | 4 | Web3 / DeFi 安全 |
| `scan` | 6 | 13 | 临时评估工具（用完即删） |

</details>

每个 bundle 包含的完整列表：**[docs/BUNDLES.zh.md](docs/BUNDLES.zh.md)** · 完整依赖链路：**[docs/DEPENDENCIES.zh.md](docs/DEPENDENCIES.zh.md)**
...
默认作用域是当前项目。加 `--to global` / `--from global` 才操作全局。

```bash
ecc-tailor add skill <name>                              # 给当前项目加 skill
ecc-tailor add bundle <name>                             # 给当前项目加 bundle
ecc-tailor add mcp <name>                                # 给当前项目加 MCP server
ecc-tailor add context <name> --to global                # 全局加 context
ecc-tailor add skill <name> --to global                  # 全局加 skill
ecc-tailor remove skill <name>                           # 从当前项目移除
ecc-tailor remove mcp <name> --from global               # 全局移除 MCP server
```
...
ecc-tailor inventory --type context                      # 全部 context
...
ecc-tailor add bundle java-proj                          # 根据建议装
...
- **Symlink** — agent 和 rule 按文件链接，skill 按目录链接，指向 ECC clone
...
- **Context** — ECC 对话预设 symlink 到 `~/.claude/contexts/`；在对话中用 `@contexts/dev` 引用，切换 Claude 工作模式（dev / review / research）；不自动加载，零开销
...
`~/.config/ecc-tailor/config.json` — 大部分字段可选，有合理默认值：
...
| `global.extras.*` | bundle 之外额外加的 agent/skill/command/context/mcp；`rulesLanguages` 用于补充额外规则 |
| `global.excludes.*` | 从 bundle 里排除的 |
| `projects[].bundles` | 数组，一个项目可吃多个 bundle |
| `bundleOverrides` | 按 bundle 定制（排除/添加 agent、skill、mcp、rules、commands） |
